...
**Zen Browser**
1. 按照 Firefox 的方式安装扩展后
2. 点击新建标签页并输入 `about:config`
3. 搜索 `zen.urlbar.replace-newtab`
4. 将该选项设置为 `false`（禁用）
...
**如何备份数据？**
1. **空间配置**：右键 Dock 栏最右侧的空间切换按钮 → 选择“导出空间”→ 保存 JSON 文件
2. **贴纸内容**：
   - 文字贴纸：右键贴纸 → “导出为图片”
   - 图片贴纸：右键贴纸 → “复制图片”或“导出 PNG”
3. **壁纸**：自己上传的壁纸建议保留原始文件

**如何恢复数据？**
1. **空间配置**：右键 Dock 栏最右侧的空间切换按钮 → 选择“导入空间”→ 选择之前导出的 JSON 文件
2. **贴纸内容**：使用 `Ctrl+V` 粘贴之前保存的图片
3. **壁纸**：在设置中重新上传壁纸

### ⚙️ 权限说明

**为什么需要“主机权限”？**
- 搜索建议功能需要访问搜索引擎 API（如 Google Suggest）
- 这是可选权限，不开启也可以正常使用所有其他功能
- 开启方法：浏览器扩展管理页面 → Eclipse Tab → 权限 → 开启“主机权限”

### 📊 使用限制

**图片大小有限制吗？**
- 贴纸图片：单张图片建议不超过 5MB，过大可能影响性能
- 壁纸：支持 10MB+ 高清图片，会自动压缩存储
- 支持格式：JPG、PNG、GIF、WebP

**贴纸数量有限制吗？**
- 没有硬性限制，但建议不要超过 50 个贴纸
- 过多贴纸可能影响页面性能
