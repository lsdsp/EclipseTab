...
- 安全约束强化：Google 固定第一位且不可删除，不允许将已保存引擎删到 0 个。
- 搜索体验优化：设置面板新增“新标签页”开关，控制搜索结果打开方式。
- 编辑模式增强：`Esc` 默认全局可退出编辑模式；编辑模式下左键单击文字贴纸可直接编辑。
- 贴纸配置增强：新增“字体”配置，支持“手写 / 普通 / 代码”三种字体预设。
...
- **专注创作**：隐藏所有交互提示，纯净画布
- **快速退出**：`Esc` 一键退出，或再次右键切换回正常模式

**⌨️ 快捷操作**
- `双击背景`：快速添加文字贴纸
- `编辑模式 + 左键单击文字贴纸`：直接编辑文字贴纸
- `Ctrl+V`：粘贴图片
...
- **快速切换**：点击搜索引擎图标即可切换
- **自定义引擎**：支持添加无限数量的引擎
- **URL 占位符**：支持 `{query}` 与 `%s`
- **编辑模式管理**：全局编辑模式下可删除已保存引擎（Google 固定第一且不可删除）
- **持久化**：自动保存选择与引擎列表

#### 搜索行为
- **新标签页开关**：设置中可切换搜索结果在“当前标签页 / 新标签页”打开
...
**编辑文字贴纸**
1. 右键点击文字贴纸并选择 "Edit"，或在编辑模式下左键单击文字贴纸
2. 进入输入态编辑内容
3. 修改文字内容或样式
4. 确认保存
...
**退出创意模式**
- 按 `Esc` 键，或再次右键选择 "Toggle Edit Mode"
...
1. 点击搜索框输入关键词
2. 按 Enter 触发搜索
3. 按“新标签页”设置在当前标签页或新标签页打开结果

**切换搜索引擎**
1. 点击搜索引擎图标
2. 选择目标引擎
3. 进入全局编辑模式后可删除已保存引擎（Google 不可删除）
4. 自动保存选择与引擎列表
